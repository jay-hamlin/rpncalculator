...
module rpn_calc(profile) {
  ...
  translate_j(0.00, -1.00,(-4*heightsIncrement)) g20_row(w=keyx,h=keyy,n=3) legend(legendsB[0],size=8) cherry() key(false); // LEFT ARROW
  ...
  for(j = [0:4]) {
    for (i = [0:4]) {
        if((i==4)&&(j==4)){
            ...
            translate_j(i, (2-j),sparOffsetFromFloor) squiggleHoriz(-1*(gridx/2),(gridy/2));
        }
    }
   }
