...
keyx = (12.00-0.10);  // width of a key in mm
keyy = (7.00-0.10);  // height of a key in mm
...
module key(inset = false) {
 ...
 // JFH adding a brim around the bottom to hold key in enclosure
     translate([0,0,0.5]){
     difference(){
        cube([keyx-5,keyy+1.5,1], center=true);
        cube([keyx-4.98,keyy-1.5,1.125], center=true);
      }
     }
...
module rpn_calc(profile) {
  ...
  legendsD = ["BASE","Send","Copy","Paste","Func"];
   ...
   doSectionA = 1;  // left row
   doSectionB = 1;  // right rows
  ...
  if(doSectionB){
       for(j = [0:3]) {
         for (i = [0:4]) {
            ...
            z = 0.00;
            if(idx == 0)
                translate_j(i,j,z) g20_row(w=keyx,h=keyy, n=3) legend(legendsA[idx],size=$font_size) cherry() key(false); // "MOD"
            else
                translate_j(i,j,z) g20_row(w=keyx,h=keyy, n=3) legend(legendsA[idx],size=$font_size) cherry() key(false); 
          }
        }
      
      translate_j(0.00, -1.00,0) g20_row(w=keyx,h=keyy,n=3) legend(legendsB[0],position=[0,0.30],size=8) cherry() key(false); // LEFT ARROW
      translate_j(1.00, -1.00,0) g20_row(w=keyx,h=keyy,n=3) legend(legendsB[1],size=$font_size) cherry() key(false); // Zero
      translate_j(2.00, -1.00,0) g20_row(w=keyx,h=keyy,n=3) legend(legendsB[2],size=($font_size+0.5)) cherry() key(false);  // POINT - make the dp a little larger.
      translate_j(3.50 ,-1.00,0) g20_row(w=(gridx+keyx),h=keyy,n=3) legend(legendsB[3],size=$font_size) cherry() key(false);// "Enter"
        
     for(j = [0:4]) {
        z = 0.00;
        translate_j(-1,(j-1),z) g20_row(w=keyx,h=keyy,n=3) legend(legendsC[4-j],size=3) cherry() key(false);
     }
  }
  ...
  if(doSectionA){ 
         for(j = [0:4]) {
            z = 0.00;
            translate_j(-2.5,(j-1)) g20_row(w=(1.50 * keyx),h=keyy,n=3) legend(legendsD[4-j],size=$font_size) cherry() key(false);
